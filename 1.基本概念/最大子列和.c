...
int Max3(int A, int B, int C) // line 33-83 ：分治法
{                             /* 返回3个整数中的最大值 */
...
int MaxSubseqSum4(
    int a[],
    int n) { //在线处理,"在线"的意思是指每输入一个数据就进行即时处理，在任何一个地方终止输入，算法都能正确给出当前的解
  int ThisSum, MaxSum;
  int i;
  ThisSum = MaxSum = 0;
  for (i = 0; i < n; i++) {
    ThisSum += a[i]; //向右累加
    if (ThisSum > MaxSum)
      MaxSum = ThisSum;   //发现最大的和则更新当前和
    else if (ThisSum < 0) //如果当前子列和为负
      ThisSum = 0;        //则不可能使后面的子列和增大，舍弃
  }
  return MaxSum;
}
